        ...
        <div class="right-content">
            <form id="right-wrapper" action="{{ route('member.publishing.channelDemand.store') }}" method="post">
                ...
                <div class="one-demand-content">
                    ...
                    <div class="one-demand relative">
                        @foreach($product_config['channel'] as $key => $value)
                            <input type="radio" name="channel_id" value="{{ $key }}">{{ $value }}
                        @endforeach
                    </div>
                </div>
                <div class="one-demand-content">
                    ...
                    <div class="one-demand relative">
                        <input type="text" class="inputlist" name="title" placeholder="请输入产品名">
                    </div>
                </div>
                <div class="one-demand-content" style="height: auto">
                    ...
                    <label for="{{$key}}" style="display:inline-block;margin:0 10px;width: 60px;height: 30px;"><input type="checkbox" name="game_type_id[]" id="{{ $key }}" value="{{ $key }}">{{ $value }}</label>
                    @endforeach
                    </div>
                </div>
                <div class="one-demand-content">
                    ...
                    <div class="one-demand relative">
                        @foreach($product_config['channel_platform'] as $key => $value)
                           <input type="checkbox" name="platform_id[]" id="{{ $key }}" value="{{ $key }}">{{ $value }}
                        @endforeach
                    </div>
                </div>
                <div class="one-demand-content">
                ...
                <div class="one-demand-content">
                    ...
                    <div class="one-demand relative">
                        @foreach($product_config['type'] as $key => $value)
                            <input type="checkbox" name="type_id[]" id="{{ $key }}" value="{{ $key }}">{{ $value }}
                        @endforeach
                    </div>
                </div>
                <div class="one-demand-content">
