            ...
            <form id="right-wrapper" action="{{ route('member.publishing.issueDemand.store') }}" method="post">
                ...
                <div class="one-demand-content" style="height: auto">
                    ...
                    @foreach($product_config['game_type_issue'] as $key => $value)
                        <label style="width: 60px;display: inline-block"><input type="radio" name="game_type_id" value="{{ $key }}">{{ $value }}</label>
                    @endforeach
                    </div>
                </div>
                ...
                <div class="one-demand-content">
                    ...
                    <div class="one-demand relative">
                        @foreach($product_config['cooperation'] as $key => $value)
                            <input type="checkbox" name="cooperation_id[]" id="p{{$key}}" value="{{ $key }}" style="margin-left: 20px"><label for="p{{$key}}">{{ $value }}</label>
                        @endforeach
                    </div>
                </div>
                <div class="one-demand-content">
                ...
                <div class="one-demand-content"  style="height: auto">
                    ...
                    <div class="one-demand relative"  style="height: auto">
                        <textarea class="introduction" name="description" placeholder="请输入需求介绍"  rows="12" cols="70" contenteditable="true"></textarea>
                    </div>
                </div>
                <div class="one-demand-content" style="height: auto">
                    <div class="one-demand-name">
