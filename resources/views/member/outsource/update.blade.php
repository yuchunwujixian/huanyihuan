        ...
        <div class="right-content">
            <form id="right-wrapper" action="{{ route('member.publishing.outsource.store') }}" method="post">
                ...
                <div class="one-demand-content">
                    ...
                    <div class="one-demand relative">
                        @foreach($product_config['outsource'] as $key => $value)
                            <input type="radio" name="outsource_id" value="{{ $key }}" @if($key == $data->outsource_id) checked @endif>{{ $value }}
                        @endforeach
                    </div>
                </div>
                <div class="one-demand-content">
                    ...
                    <div class="one-demand relative">
                        <input type="text" class="inputlist" name="title" placeholder="请输入渠道名" value="{{ $data->title }}">
                    </div>
                </div>
                <div class="one-demand-content" style="height: auto">
                    ...
                    <div class="one-demand relative" style="height: auto">
                        @foreach($product_config['precondition'] as $key => $value)
                            <input type="radio" name="precondition_id" value="{{ $key }}" @if($key == $data->precondition_id) checked @endif>{{ $value }}
                        @endforeach
                    </div>
                </div>
                <div class="one-demand-content">
                ...
                <div class="one-demand-content">
                    ...
                    <div class="one-demand relative">
                        <input type="text" class="inputlist" name="needs" placeholder="请简要说明一下供需" value="{{ $data->needs }}">
                    </div>
                </div>
                <div class="one-demand-content"  style="height: auto">
                    <div class="one-demand-name">
                    ...
                    <div class="one-demand relative"  style="height: auto">
                        <textarea class="introduction" name="description" placeholder="请输入外包介绍"  rows="12" cols="70" contenteditable="true">{{ $data->description }}</textarea>
                    </div>
                </div>
