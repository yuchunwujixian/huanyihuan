            ...
            <div id="right-wrapper">
                ...
                @if($is_register_company_info)
                    ...
                    <div class="go-release-btn">
                            <a href="{{route('member.publishing.outsource.create')}}">
                                <span>发布外包</span>
                        ...
                        @foreach($data as $value)
                            <div class="one-item" style="position: relative">
                                ...
                                <div class="one-item-content" style="position: relative">
                                    <p>
                                        {{$product_config['outsource'][$value['outsource_id']]}} |
                                        {{$product_config['precondition'][$value['precondition_id']]}}
                                    </p>
                                    <p>联系人信息 ： {{$value->contact}} -- {{$product_config['tel_type'][$value->tel_type_id]}} : {{$value->telephone}}</p>
                                    <p>{{$value->updated_at}}</p>
                                    @if($value['status'] != -1)
                                    <div class="operation">
                                        <a class="inline-block" href="{{ route('member.publishing.outsource.update', ['id' => $value->id]) }}">
    ...
    <script>
        $('.del').click(function(){
            if(confirm("确定要删除数据")){
                ...
                $.ajax({
                    url:"{{ route('member.publishing.outsource.destroy') }}",
                    type:"POST",
                    dataType : 'json',
